...
# Color blindness conversion functions
...
def generate_frames(blindness_type):
    ...
    if not cap.isOpened():
        ...
        while True:
            #ret is true if frame is captured successfully and frame contains the captured frames
            ret, frame = cap.read()
            if not ret:
...
# Route to render the first HTML page
@app.route('/')
def index():
    return render_template('homepage.html')

# Route to render the second HTML page and handle color blindness simulation
...
# Route to get color name based on RGB values
